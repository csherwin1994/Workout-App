    ...
    var body: some View {
        ZStack {
            ...
            List {
                ...
                Section {
                    HStack(spacing: 0) {
                        SummaryCell(value: session.formattedDuration, label: "Duration", icon: "clock.fill", color: IronColor.accent)
                        Divider().frame(height: 40)
                        SummaryCell(value: "\(session.totalSets)", label: "Sets", icon: "dumbbell.fill", color: IronColor.accentPurple)
                        Divider().frame(height: 40)
                        SummaryCell(
...
struct SummaryCell: View {
    ...
    var body: some View {
        VStack(spacing: 4) {
            ...
            Text(value).font(.system(size: 15, weight: .bold)).foregroundStyle(IronColor.label)
            Text(label).font(.system(size: 10)).foregroundStyle(IronColor.labelTertiary)
        }
