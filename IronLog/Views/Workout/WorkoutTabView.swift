...
struct WorkoutTabView: View {
    ...
    private var heroSection: some View {
        ZStack(alignment: .bottomLeading) {
            RoundedRectangle(cornerRadius: IronSpacing.radiusLg)
            ...
            HStack(spacing: 10) {
                ...
                Button { showingAIGenerator = true } label: {
                    ...
                    .overlay(RoundedRectangle(cornerRadius: IronSpacing.radius)
                        .stroke(IronColor.accentPurple.opacity(0.3), lineWidth: 0.5))
                }
    ...
    private func miniStat(value: String, icon: String) -> some View {
        HStack(spacing: 4) {
            Image(systemName: icon).font(.caption2).foregroundStyle(IronColor.labelTertiary)
            Text(value).font(.caption.weight(.medium)).foregroundStyle(IronColor.labelSecondary)
        }
    }
}
...
struct ActiveWorkoutBar: View {
    ...
    var body: some View {
        HStack(spacing: IronSpacing.md) {
            HStack(spacing: 8) {
                Circle()
                    .fill(IronColor.success)
                    .frame(width: 8, height: 8)
                    .shadow(color: IronColor.success, radius: 4)
                VStack(alignment: .leading, spacing: 1) {
                    Text(vm.session?.title ?? "Active Workout")
                        .font(.subheadline.weight(.semibold))
                        .foregroundStyle(IronColor.label)
                    Text(vm.formattedElapsed)
                        .font(IronFont.mono(13))
                        .foregroundStyle(IronColor.labelSecondary)
                }
            }
            Spacer()
        ...
        .padding(.vertical, 12)
        .background(IronColor.surface)
        .clipShape(RoundedRectangle(cornerRadius: IronSpacing.radiusLg))
        .overlay(RoundedRectangle(cornerRadius: IronSpacing.radiusLg)
            .stroke(IronColor.success.opacity(0.4), lineWidth: 1))
