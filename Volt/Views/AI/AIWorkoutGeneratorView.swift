    ...
    var body: some View {
        ScrollView {
            VStack(spacing: VoltSpacing.xl) {
                        ...
                        ForEach(durations, id: \.self) { d in
                            Button {
                                ...
                                .overlay(RoundedRectangle(cornerRadius: VoltSpacing.radiusSm)
                                    .stroke(duration == d ? Color.clear : VoltColor.border, lineWidth: 0.5))
                            }
                        }
                    }
                }
                    ...
                    HStack(spacing: 10) {
                        ForEach(EquipmentLevel.allCases) { eq in
                            Button {
                                ...
                                .frame(maxWidth: .infinity)
                                .frame(height: 70)
                                .background(equipment == eq ? VoltColor.accentDim : VoltColor.surface)
                                .foregroundStyle(equipment == eq ? VoltColor.accent : VoltColor.labelSecondary)
                                .clipShape(RoundedRectangle(cornerRadius: VoltSpacing.radiusSm))
                                .overlay(RoundedRectangle(cornerRadius: VoltSpacing.radiusSm)
                                    .stroke(equipment == eq ? VoltColor.accent.opacity(0.4) : VoltColor.border, lineWidth: 0.5))
                            }
                        }
                    }
                }
                        ...
                        ForEach(ExperienceLevel.allCases) { lvl in
                            Button {
                                ...
                                Text(lvl.rawValue)
                                    ...
                                    .frame(maxWidth: .infinity)
                                    .frame(height: 44)
                                    .background(experience == lvl ? VoltColor.surface : VoltColor.surfaceHigh)
                                    .foregroundStyle(experience == lvl ? VoltColor.label : VoltColor.labelSecondary)
                                    .clipShape(RoundedRectangle(cornerRadius: VoltSpacing.radiusSm))
                                    .overlay(RoundedRectangle(cornerRadius: VoltSpacing.radiusSm)
                                        .stroke(experience == lvl ? VoltColor.accent : VoltColor.border, lineWidth: experience == lvl ? 1 : 0.5))
                            }
                        }
                    }
                }
                ...
                // Generate button
                VoltButton("Generate My Workout", icon: "sparkles", action: onGenerate)
                    .padding(.horizontal, VoltSpacing.md)
        ...
        Button(action: action) {
            VStack(spacing: 8) {
                ...
                Text(focus.rawValue)
                    ...
                    .foregroundStyle(isSelected ? .white.opacity(0.9) : VoltColor.labelSecondary)
                    .multilineTextAlignment(.center)
                    .lineLimit(2)
            }
            .frame(maxWidth: .infinity)
...
private struct ResultView: View {
    ...
    private var actionButtons: some View {
        VStack(spacing: 10) {
            VoltButton("Start Workout Now", icon: "play.fill", action: onStartWorkout)
            VoltButton("Save as Routine", icon: "bookmark.fill", style: .secondary, action: onSaveRoutine)
            VoltButton("Regenerate", icon: "arrow.clockwise", style: .ghost, action: onRegenerate)
        }
        .padding(.horizontal, VoltSpacing.md)
        .padding(.bottom, VoltSpacing.xl)
    }
...
private struct MissingKeyCard: View {
    var body: some View {
        ...
        .padding(VoltSpacing.xl)
        .voltCard(padding: VoltSpacing.xl)
        .padding(.horizontal, VoltSpacing.md)
    }
}
    ...
    var body: some View {
        VStack(spacing: VoltSpacing.md) {
            ...
            Text(message)
                ...
                .multilineTextAlignment(.center)
            HStack(spacing: 10) {
                VoltButton("Try Again", icon: "arrow.clockwise", action: onRetry)
                VoltButton("Back", style: .secondary, action: onBack)
            }
        }
        .padding(VoltSpacing.xl)
        .voltCard(padding: VoltSpacing.xl)
...
private struct WorkoutPreviewCard: View {
    ...
    var body: some View {
        ...
        .voltCard(padding: VoltSpacing.md)
        .padding(.horizontal, VoltSpacing.md)
    }
